...
description: When objects move at constant speed through deep water, the intricate wave pattern can be derived by noticing that only waves stationary relative to the object persist.
author: Ziyue Li
toc: true
comments: true
layout: post
...
In Feynman's lecture [^1], he mentioned that if one is clever enough, one can derive the Kelvin wake pattern from the fact that the pattern of the waves is stationary to the constant velocity of the boat; any other pattern would get lost from the boat.
This post lays out the details of this derivation.


## Phase Velocity and Group Velocity
This post will assume a basic understanding of the phase velocity and group velocity.
...
The group velocity goes half as fast as the phase.
...
Notice that the phase velocity is twice of the group velocity.
...
1. **A general sketch of wavefronts for a certain wave length**
    ...
    For a certain wavelength, AD is where the wavefront would be for the phase velocity and because the group velocity is half of the phase velocity, AC is where the group of waves actually propagates to.
    The green lines are all parallel to AC and mark the phase wavefront on AC.
...
3. **Connect "stationary" waves that are at the same phase to form equal-phase wavefronts**
    ...
    In order to derive the wave pattern, we need to trace out "equal-phase lines" that connect wavefronts of the same phase.
...
4.  **Composite all equal phase lines that are separated by $2\pi$**
    ...
    Since the waves at the front of the boat (point A) is always at the peak, which has the phase of 0, the peaks of waves are at $\phi = 2\pi n$, where $n$ is any positive integer.
...
For Kelvin Wake pattern construction that takes into account the size of the boat, the readers are referred to this more advanced research [^2].
