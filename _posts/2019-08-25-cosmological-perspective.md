...
One way to imagine this is to think of space like a rubber sheet. It can stretch and contract while the objects are moving on it. The fact that the space can stretch and contract induces an apparent velocity even when the object is not moving relative to the space by itself. Imagine two fixed points on the rubber sheet. The stretching and contracting of the rubber makes one move away from or towards the other. On top of this apparent motion, the relative movement with respect to the local space induces the "local velocity". And the total effective velocity of an object seen from another object will be the addition of these two motions.
...
The lattice spacing between the coordinates indicates how the space itself is changing in time.
And the change in the co-moving coordinates indicates a real motion relative to the local space.
We denote the co-moving coordinates with letter $r$ and the lattice spacing with $a(t)$, as a function of time.
...
**This introduces the first thought experiment: in a universe where space varies with time, the proper distance an object travelled from point A at time $t_{A}$ to point B at time $t_{B}$ is in general not the same as the proper distance at time $t_A$ nor $t_B$.**

Let's say the space is expanding. As the object travels from A to B, the space in front of it is constantly increasing, so that the distance it has to travel to reach B is certainly larger than the original proper distance at time $t_{A}$. At the same time, the space behind it, i.e. the space it has already travelled, is also constantly expanding, so that the proper distance at time $t_{B}$, namely $D_{P}(t_{B})=a(t_{B})\|r_{B}-r_{A}\|$, is larger than the distance it actually traversed. So in this case we have $D_{P}(t_A)<D_{T}<D_{P}(t_B)$, where we denoted the traveling distance with $D_{T}$ and the proper distance between AB at time $t_A$ and $t_B$ with $D_{P}(t_A)$ and $D_{P}(t_B)$.
...
<figure role="presentation">
  ...
  <figcaption>
    Fig. 1 A view of an isometric embedding of part of the visible universe over most of its history. The vertical direction is the time and the blue circles represent the space at a certain cosmological time. This figure shows how a light ray (red line) can travel an effective distance of 28 billion light years (orange line) in just 13 billion years of cosmological time. Here the 13 billion light years is the traveling distance and the 28 billion light years is the proper distance between AB at the end time. [This demonstration is taken from the section "Measuring distances in expanding space" in this wikipage: <a href="https://en.wikipedia.org/wiki/Metric_expansion_of_space">Metric expansion of space</a>].
  </figcaption>
</figure>


**Once you figure this out, it's easy to understand the following seemingly counter-intuitive phenomena**:
...
Whether the expansion of the universe is accelerating or decelerating is defined by $\ddot{a}(t)$. This is obviously defined from the perspective that if we observe the space expansion between two fixed points, whether we see the rate of space added is increasing or decreasing. But if we think about this for a minute, we will realize that more spaces may be added later, simply because there are more spaces between the two points later.
In other words, the textbook definition of "accelerating expansion" is the "apparent accelerating expansion".

This definition is very unfortunate, since it does not capture the "intrinsic expansion rate" of the universe and thus induces a lot of confusions, especially when we consider the event horizon of the universe, as we will see below.

## Intrinsic Expansion Rate: Hubble's constant

In order to get the "intrinsic expansion rate", we need to remove the dependence on the changing distance. This can be achieved by dividing the apparent velocity of a distant co-moving object by the proper distance in between:

$$H=\dfrac{r\dot{a}(t)}{ra(t)}=\dfrac{\dot{a}(t)}{a(t)},$$

where $\dot{a}(t)$ is the time derivative of $a(t)$.
For those interested, the Young's modulus in solid state physics is defined in a similar way to get rid of the length dependence so that it reflects the property of the material.

In other words, Hubble's constant gives the true expansion rate.

To make it clearer why using $\ddot{a}(t)$ to understand expansion is a bad idea, think of it this way: let's say the Hubble's constant is not changing with time, which means the characteristic expansion of the universe is not speeding up nor slowing down. But since at a later time, $a(t)$ itself becomes larger, the induced apparent velocity at that larger proper distance will become larger, and thus $\ddot{a}(t)>0$, and we get the conclusion that the universe' expansion is "accelerating".
That "acceleration" is relative to the coordinate system set up from the very beginning.
If you compare the expansion of space now to the expansion of space between objects with the same amount of proper distance in between a long time ago, the expansion is actually slower.

How does $a(t)$ change is the intrinsic expansion rate is constant?
We can solve $H=\dot{a}(t)/a(t)=\beta$ to get
$$a(t)=e^{\beta t}.$$
In other words, at constant intrinsic expansion rate, the $a(t)$ should be increasing exponentially.
...
Said in another way, a constant $H$ corresponds to exponentially increasing $a(t)$, while a decreasing $H$ corresponds to slower increase in $a(t)$.
...
However, due to the space expansion, we can actually see objects that are currently moving with speed above the speed of light, beyond the Hubble distance, as explained below.


## Cosmic Event Horizon

Event horizon is the distance at which the photon emitted now will never reach us in the infinite future.
...
Actually, we will be able to see light emitted from some objects outside of our Hubble Sphere, thanks to the slowing down of the intrinsic expansion rate $H$!

### A Detour

This is very counter-intuitive.
Before we go into the correct explanation, I want to spell out a couple common traps people tend to fall into when thinking about this problem:
...
1. As hinted above, people assume that just because the photons emitted from those objects are now apparently moving away from us, and the expansion is "accelerating", there's no way that the light emitted from these objects will reverse their course and come towards us.
    ...
    Just because at one moment this values is positive (moving away from us), does not mean that it will stay that way, because the co-moving distance $r$ is decreasing due to the fact that the photon is locally heading in our direction.
    Therefore, the apparent velocity can potentially decrease if the decreasing $r$ is bringing down the total apparent velocity, despite the fact that $\dot{a}(t)$ is increasing.

2. Once we understand how the apparent velocity also depends on the decreasing co-moving distance, people might assume that just because the apparent velocity might start decreasing, then that is the reason that the photon can reach us.

    Unfortunately, the decrease of the apparent velocity of a photon can slow down and never get to the point where the apparent velocity is flipped.
    Although it's hard to show this without invoking some math, one can conceptually understand that it is possible that the changes of $r(t)$ and $\dot{a}(t)$ can result in the apparent velocity $r(t) \cdot \dot{a}(t)-c$ decreasing but never reaching zero.

### Explanation

It's easy to see that reasoning using the apparent velocity is difficult and error-prone.
Our first instinct and intuition leads us that way, probably because we are so used to thinking about motions in terms of how velocity changes.
However, the dynamics in an expanding universe is more complex, because the one velocity induced by the expansion is location specific (it's a function of relative distance), while the other local velocity is a constant relative to space.
...
During a small time interval $dt$, the photon travels $cdt$ distance in space, which is $dr = c dt/a(t)$ in terms of the co-moving distance.
This is also the largest co-moving distance from which we can still observe the object.
The corresponding total proper distance is therefore the Event Horizon:
...
As mentioned before, the intrinsic expansion rate of our universe is slowing down, resulting in the fact that we are able to observe objects outside of our Hubble Sphere, even though the expansion rate as measured by $a(t)$ is speeding up.

## Particle Horizon

Particle horizon describes the furthest distance we can see right now.
...
It's very important to stress that we are not seeing what these objects look like right now, since it takes time for the photon to travel to us. We are only seeing them as their younger selves when the distance to them were different.
...
This means, at the very far distant future, these objects will seem to be frozen at the time when they cross the event horizon, and we actually never see them cross the event horizon.
...
When we say the objects seem to be frozen at the event horizon, we mean, when we look out into space, the younger selves of distant objects whose light reaches us slowly approaches the time when they crossed the event horizon, but never get there.
Therefore, we should always be able to see distant objects as their younger selves moving closer and closer to the event horizon, if we still exist.
...
This is the real reason why we won't be able to these distant objects anymore, not because they already moved past the event horizon.

Does that mean that the sky will look black in the distant future?
Not if stars in our galaxy and nearby galaxies are still shining for some reason. Galaxies that are close enough to be gravitationally bound to our own when the universe expands will not be pushed away by the expansion, and will still be there.
But then again, in the far far distant future, all stars might have already died. In that case, we won't be able to see anything anymore.