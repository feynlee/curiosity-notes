...
One way to imagine this is to think of space as a rubber sheet. It can stretch and contract while the objects are moving on it. The fact that space can stretch and contract induces an apparent velocity even when the object is not moving relative to space by itself. Imagine two fixed points on the rubber sheet. The stretching and contracting of the rubber make one move away from or towards the other. On top of this apparent motion, the relative movement with respect to the local space induces the "local velocity". And the total effective velocity of an object seen from another object will be the addition of these two motions.
...
Whether the expansion of the universe is accelerating or decelerating is defined by $\ddot{a}(t)$. This is defined from the perspective that if we observe the space expansion between two fixed points, whether we see the rate of space added is increasing or decreasing. But if we think about this for a minute, we will realize that more spaces may be added later, simply because there are more spaces between the two points later.
...
If you compare the expansion of space now to the expansion of space between objects with the same amount of proper distance in between a long time ago, the expansion can be slower.

How does $a(t)$ change if the intrinsic expansion rate is constant?
We can solve $H=\dot{a}(t)/a(t)=\beta$ to get
$$a(t)=e^{\beta t}.$$
In other words, at a constant intrinsic expansion rate, the $a(t)$ should be increasing exponentially.
...
1. As hinted above, people assume that just because the photons emitted from those objects are now apparently moving away from us, and the expansion is "accelerating", there's no way that the light emitted from these objects will reverse their course and come towards us.
    ...
    Just because at one moment this value is positive (moving away from us), does not mean that it will stay that way. At the same time that $a(t)$ is increasing, the co-moving distance $r$ is decreasing because the photon is locally heading in our direction. The apparent velocity can potentially decrease if the decreasing $r$ is bringing down the total apparent velocity, even though $\dot{a}(t)$ is increasing.

2. Once we understand how the apparent velocity also depends on the decreasing co-moving distance, people might assume that just because the apparent velocity might start decreasing, then that guarantees the photon will reach us.
...
However, the dynamics in an expanding universe are more complex, because the one velocity induced by the expansion is location-specific (it's a function of relative distance), while the other local velocity is a constant relative to space.
...
This is also the largest comoving distance from which we can still observe the object.
...
As mentioned before, the intrinsic expansion rate of our universe is slowing down, resulting in the fact that we can observe objects outside of our Hubble Sphere, even though the expansion rate as measured by $a(t)$ is speeding up.
...
**The "observable universe" is defined by the particle horizon. This is the 93 billion light-years value quoted in the top picture of this post. According to Fig. 2, the size of the "observable universe" is getting bigger and bigger.**
...
This means, in the very far distant future, these objects will seem to be frozen at the time when they cross the event horizon, and we never see them cross the event horizon.
